...
async function create(userParam) {
  ...
  const mailOptions = {
    from: SES_EMAIL,
    to: user.email,
    subject: `sll-ecommerce: account verification`,
...
async function changeDefaultAddress(token, id) {
  ...
  await User.updateOne(
    { _id: userId, "addresses.defaultAddress": "true" },
    {
      $set: {
...
async function getUserAddresses(token) {
  ...
  const addresses = await User.find({ _id: ObjectId(userId) }, [
    "addresses",
    "-_id",
  ]);
...
async function resendVerify({ emailParam }) {
  ...
  const mailOptions = {
    from: SES_EMAIL,
    to: user.email,
    subject: `sll-ecommerce: account verification (resent)`,
    token: user.verificationToken,
    templateH1: `verification`,
    templateText: "verify your email",
  };
...
async function forgotPasswordRequest({ email }) {
  ...
  const mailOptions = {
    from: SES_EMAIL,
    to: user.email,
    subject: `sll-ecommerce: password recovery`,
